...
contract('Base-Fabric Integration', (accounts) => {
    ...
    describe('Register Chain', () => {
        it('Successful Register Chain', async () => {
            ...
            let start = Date.now()
            txToBenchmark = await validation.RegisterChain(TESTCHAINID, storage.address);
    ...
    describe('Add Block', () => {
        it('Successful Add Block', async () => {
            ...
            let start = Date.now()
            txToBenchmark = await validation.SubmitBlock(TESTCHAINID, rlpEncodedBlock, storage.address);
            duration = Number( (Date.now() - start) / 1000 ).toFixed(3)

            console.log("\tGas used to store fabric block: %d", txToBenchmark.receipt.gasUsed);
    ...
    describe('Chaincode usage Contract', () => {
        ...
        it('Submit Block, retrieve state and execute', async () => {
            ...
            let start = Date.now()
            txToBenchmark = await functionContract.retrieveAndExecute(TESTCHAINID, TESTDATA[0].channelId, TESTDATA[0].blocks[0].transactions[0].nsrw[0].writesets[0].key);
            duration = Number( (Date.now() - start) / 1000 ).toFixed(3)

            event = txToBenchmark.logs.some(l => { return l.event == "State" });
            assert.ok(event, "Executed event not emitted");

            log = txToBenchmark.logs.find(l => l.event == "State");
