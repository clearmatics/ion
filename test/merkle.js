...
const hash = n => new BN(Web3Utils.sha3(toHex(n)))
const merkleHash = n => bitClear(hash(n), 0xFF)
const rawDataMerkleHash = data => bitClear(new BN(Web3Utils.sha3(data)),0xFF)
...
const createMerkle = (items) => {
  const leafHash = items
    .map((leaf) => rawDataMerkleHash(leaf))
    .sort((a,b) => (new BN(a)) - (new BN(b)))
  if(leafHash.length % 2 !== 0) leafHash.push(rawDataMerkleHash('merkle-tree-extra'))
  const tree = [ leafHash ]
...
const pathMerkle = (leaf,tree) => {
  const leafHash = rawDataMerkleHash(leaf)
...
const proofMerkle = (leaf, path, root, hashLeaf,debug) => {
  const leafHash = hashLeaf ? leaf : rawDataMerkleHash(leaf)
  const result = path
    .reduce((prev,item) => {
