...
library Merkle
{
    ...
    */
    ...
    function Verify2( uint256 root, uint256 leaf_hash, uint256[] path )
        internal //pure
        returns (bool)
    {
        if( leaf_hash & ONE_SHL_255 > 0 )
        ...
        uint256[10] memory all_nodes;
        uint idx = 0;
        all_nodes[idx++] = leaf_hash;


        for( uint256 i = 0; i < path.length; i++ )
        {
            item = path[i];
            uint256 old_node = node;

            if( (item & ONE_SHL_255) > 0 ) {
                node = Hash(node, item ^ ONE_SHL_255);
                HashEvent(( (item & ONE_SHL_255) > 0 ),uint256(keccak256(item, old_node)), old_node, item ^ ONE_SHL_255, node);
            }
            else {
                node = Hash(item, node);
                HashEvent(( (item & ONE_SHL_255) > 0 ),uint256(keccak256(item, old_node)), item,old_node, node);
            }
            all_nodes[idx++] = node;
            Test(( (item & ONE_SHL_255) > 0 ),node);
        }
