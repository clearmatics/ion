...
contract EventVerifier {
    /*
        ...
        returns: log (RLPReader.RLPItem[]) Decoded log object in the form [ contractAddress, topics, data ]
    ...
    */
        ...
        internal pure returns (RLPReader.RLPItem[] memory)
    {
        ...
        */
        RLPReader.RLPItem[] memory receipt = RLPReader.toList(RLPReader.toRLPItem(_rlpReceipt));
        RLPReader.RLPItem[] memory logs = RLPReader.toList(receipt[3]);
        ...
        */
        for (uint i = 0; i < logs.length; i++) {
            RLPReader.RLPItem[] memory log = RLPReader.toList(logs[i]);
            RLPReader.RLPItem[] memory topics = RLPReader.toList(log[1]);

            bytes32 containedEventSignature = RLPReader.toBytes32(topics[0]);
            if (containedEventSignature == _eventSignature) {
                // If event signature is found, check the contract address it was emitted from
                bytes20 b20_emissionSource = SolUtils.BytesToBytes20(RLPReader.toBytes(log[0]), 0);
