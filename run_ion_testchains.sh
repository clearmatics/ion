...
tmux send-keys -t chaina "sleep 5s" C-m
...
tmux send-keys -t chaina "sleep 2s" C-m
...
tmux send-keys -t chainb "sleep 2s" C-m
...
tmux send-keys -t chaina "token.mint(100000, {from: own})" C-m
tmux send-keys -t chaina "token.transfer(send, 1000, {from: own})" C-m
...
# fund the relevant accounts
tmux send-keys -t chainb "token.mint(100000, {from: own})" C-m
tmux send-keys -t chainb "token.transfer(send, 1000, {from: own})" C-m
